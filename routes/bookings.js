...
router.get('/', async function (req, res) {
    ...
    try {
        ...
        if (req.query.numTickets) {
            query.noOfTicket = parseInt(req.query.noOfTicket);
        }
        ...
        if (req.query.sort_by) {
            ...
            if (sortBy.length > 1 && ["noOfTicket"].includes(sortBy[0])) {
...
router.post('/', async function (req, res) {
    const db = await connectToDB();
    try {
        req.body.noOfTicket = parseInt(req.body.noOfTicket);
...
router.put('/:id', async function (req, res) {
    ...
    try {
        ...
        //remote the previous update id from client side body
        delete req.body._id;
...
router.get('/stats/superhero', async function (req, res) {
    ...
    try {
        ...
        if (req.query.term) {
            pipelines.push({ $match: { term: req.query.term } });
        }
